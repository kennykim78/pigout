...
const MyPage = () => {
  ...
  const loadData = async () => {
    try {
      ...
      // API에서 데이터 로드
      const [statsData, historyData, medicineData] = await Promise.all([
        getMyStatus().catch(() => ({
          totalRecords: 0,
          avgScore30Days: 0,
          recentDays: 0,
      ...
      setStats(statsData);
      setMedicines(
        Array.isArray(medicineData) ? medicineData : medicineData?.data || []
      );
    ...
    <div className="mypage">
      ...
      <section className="mypage__profile-card">
        ...
        <div className="mypage__profile-info">
          <h2 className="mypage__profile-name">
            {user?.full_name || "먹어도돼지 사용자"}
          </h2>
          <p className="mypage__profile-id">
            ID:{" "}
            {user?.username ||
              localStorage.getItem("pigout_device_id")?.substring(0, 8) ||
              "guest"}
          </p>
        </div>
      </section>
        ...
        <div className="mypage__disease-list">
          {diseases && diseases.length > 0 ? (
            diseases.map((disease, index) => (
              <span key={index} className="mypage__disease-tag">
                {disease}
              </span>
            ))
          ) : (
            ...
            medicines.slice(0, 4).map((medicine, index) => (
              <div key={medicine.id || index} className="mypage__medicine-item">
                ...
                <div className="mypage__medicine-info">
                  <div className="mypage__medicine-name">
                    {medicine.name || medicine.item_name}
                  </div>
            ...
            recentRecords.map((record) => (
              ...
              >
                <div className="mypage__recent-info">
                  ...
                  <div className="mypage__recent-time">
                    {record.date} {record.time}
                  </div>
                </div>
                <div
                  className={`mypage__recent-score mypage__recent-score--${getScoreColor(
                    record.score
                  )}`}
                >

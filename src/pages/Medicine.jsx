...
const Medicine = () => {
  ...
  const {
    medicines,
    setMedicines,
    addMedicine: addToStore,
    deleteMedicine: removeFromStore,
    isLoading,
    setLoading,
    setError,
    shouldRefetch,
  ...
  const analyzeImageWithAI = async (base64Data, mimeType) => {
    ...
    try {
      ...
      if (result.success && result.verifiedMedicines?.length > 0) {
        console.log(
          "[이미지 분석] 검증된 약품 개수:",
          result.verifiedMedicines.length
        );
  ...
  const handleAddSelectedMedicines = async () => {
    ...
    for (const medicine of medicinesToAdd) {
      try {
        ...
        console.log(
          `[이미지 등록] ${baseItemName} 상세정보 조회 시작 (itemSeq: ${baseItemSeq})`
        );
  ...
  const handleAddMedicine = async (medicine) => {
    ...
    try {
      ...
      if (result.medicineRecord) {
        const medicineTypes = JSON.parse(
          sessionStorage.getItem("medicineTypes") || "{}"
        );
        medicineTypes[result.medicineRecord.id] = isHealthFood
          ? "healthfood"
          : "medicine";
  ...
  const handleUpdateSchedule = async (medicineId, scheduleData) => {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/medicine/${medicineId}`,
        {
          method: "PATCH",
          headers: {
    ...
    const { abort } = analyzeAllMedicinesStream({
      ...
      onResult: async (data) => {
        ...
        if (data.success && data.data) {
          ...
          try {
            await logActivity("medicine_analysis", null, "약물 상호작용 분석");
          } catch (e) {
      ...
      onError: (error) => {
        ...
        if (error.message?.includes("503")) {
          ...
          userFriendlyMessage =
            "⚠️ AI 분석 요청이 일시적으로 제한되었습니다. 1-2분 후 다시 시도해주세요.";
        } else if (
  ...
  const getSafetyBadgeClass = (safety) => {
    switch (safety) {
      case "safe":
        return "medicine__safety-badge--safe";
      case "caution":
        return "medicine__safety-badge--caution";
      case "danger":
        return "medicine__safety-badge--danger";
      default:
        return "";
    }
  };
                  ...
                  <div className="medicine__analysis-content">
                    ...
                    <div className="medicine__analysis-body">
                      ...
                      <div className="medicine__overall-safety">
                        <div
                          className={`medicine__safety-badge ${getSafetyBadgeClass(
                            analysisResult.analysis.overallSafety
                          )}`}
                        >
                      ...
                      <div className="medicine__summary-section">
                        <h3>📋 종합 평가</h3>
                        <p className="medicine__summary-text">
                          {analysisResult.analysis.summary}
                        </p>
                      </div>

                      {/* 위험한 조합 */}
                      {analysisResult.analysis.dangerousCombinations &&
                        analysisResult.analysis.dangerousCombinations.length >
                          0 && (
                          <div className="medicine__danger-section">
                            ...
                            {analysisResult.analysis.dangerousCombinations.map(
                              (combo, idx) => (
                                <div
                                  key={idx}
                                  className="medicine__interaction-card medicine__interaction-card--danger"
                                >
                                  <h4>
                                    {combo.drug1} ⚡ {combo.drug2}
                                  </h4>
                                  <p className="medicine__interaction-desc">
                                    {combo.interaction}
                                  </p>
                      ...
                      {/* 주의 필요 조합 */}
                      {analysisResult.analysis.cautionCombinations &&
                        analysisResult.analysis.cautionCombinations.length >
                          0 && (
                          <div className="medicine__caution-section">
                            ...
                            {analysisResult.analysis.cautionCombinations.map(
                              (combo, idx) => (
                                <div
                                  key={idx}
                                  className="medicine__interaction-card medicine__interaction-card--caution"
                                >
                                  <h4>
                                    {combo.drug1} + {combo.drug2}
                                  </h4>
                      ...
                      {/* 긍정적 효과 */}
                      {analysisResult.analysis.synergisticEffects &&
                        analysisResult.analysis.synergisticEffects.length >
                          0 && (
                          <div className="medicine__synergy-section">
                            ...
                            {analysisResult.analysis.synergisticEffects.map(
                              (effect, idx) => (
                                <div
                                  key={idx}
                                  className="medicine__interaction-card medicine__interaction-card--safe"
                                >
                                  <h4>{effect.drugs.join(" + ")}</h4>
                                  <p className="medicine__benefit">
                                    💚 {effect.benefit}
                                  </p>
                                  <p className="medicine__interaction-desc">
                                    {effect.description}
        ...
        <div className="medicine__add-overlay">
          <div className="medicine__add-modal">
            <h3 className="medicine__add-title">
              {addProgress.medicineName || "약품"}을 추가 중입니다.
            </h3>

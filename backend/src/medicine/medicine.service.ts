...
export class MedicineService {
  ...
  async searchMedicine(keyword: string, numOfRows: number = 200) {
    try {
      ...
      // 1️⃣ 약품명으로 검색 (1차 - 우선)
      let nameResults = await this.externalApiClient.getMedicineInfo(
        keyword,
        apiLimit
      );
      ...
      const hasRealNameResults = nameResults.some(
        (item: any) =>
          item._isAIGenerated !== true &&
          item.itemSeq &&
          !item.itemSeq.startsWith("AI_")
      );

      console.log(
        `[약품 검색-디버그] hasRealNameResults: ${hasRealNameResults}`
      );
      ...
      if (!hasRealNameResults) {
        ...
        efficacyResults = await this.externalApiClient.searchMedicineByEfficacy(
          keyword,
          apiLimit
        );
        manufacturerResults =
          await this.externalApiClient.searchMedicineByManufacturer(
            keyword,
            apiLimit
          );
      } else {
        console.log(
          `[약품 검색] 약품명 성공 (${nameResults.length}건) → 효능/제조사 스킵 ⏭️ (API 절약)`
        );
      }

      console.log(
        `[약품 검색] 결과 - 약품명: ${nameResults.length}건, 효능: ${efficacyResults.length}건, 제조사: ${manufacturerResults.length}건`
      );
      ...
      const realResults = uniqueResults.filter(
        (item: any) => !isAIGenerated(item)
      );
      ...
      console.log(
        `[약품 검색] AI 데이터 필터링 후: ${realResults.length}건 (실제 약품)`
      );
      ...
      for (const result of results) {
        ...
        const fullMedicineData = finalResults.find(
          (item: any) =>
            item.itemSeq === result.itemSeq && item.entpName === result.entpName
        );
  ...
  async searchHealthFood(keyword: string, numOfRows: number = 200) {
    try {
      console.log(
        `[건강기능식품 검색] 키워드: ${keyword}, 요청 수: ${numOfRows}`
      );
      ...
      // 건강기능식품 API 검색
      let results = await this.externalApiClient.searchHealthFunctionalFood(
        keyword,
        apiLimit
      );
      ...
      const hasRealResults =
        results &&
        results.some(
          (item: any) => item.itemSeq && !item.itemSeq.startsWith("AI_HF_")
        );
      ...
      if (results && results.length > 0) {
        ...
        for (const result of formattedResults) {
          const fullData = results.find(
            (item: any) =>
              item.itemSeq === result.itemSeq &&
              item.entpName === result.entpName
          );
        ...
        console.log(
          `[건강기능식품 검색] ✅ ${formattedResults.length}건 검색됨, 캐시 저장 완료`
        );
        return formattedResults;
      }

      // 🆕 먼저 AI에게 제품 유형 판단 요청 (의약품 검색 전에!)
      const productType = await this.externalApiClient.classifyProductType(
        keyword
      );
      ...
      if (productType === "healthFood") {
        console.log(
          `[건강기능식품 검색] AI가 건강기능식품으로 판단 - AI 정보 생성`
        );
        const aiResults = await this.externalApiClient.generateAIHealthFoodInfo(
          keyword,
          10
        );
      ...
      if (productType === "medicine") {
        ...
        let medicineResults = await this.externalApiClient.getMedicineInfo(
          keyword,
          5
        );

        // 의약품 검색에서 AI 데이터 제거 (itemSeq가 AI_로 시작하거나 _isAIGenerated 플래그)
        const realMedicineResults = (medicineResults || []).filter(
          (item: any) =>
            item.itemSeq &&
            !item.itemSeq.startsWith("AI_") &&
            !item._isAIGenerated
        ...
        if (realMedicineResults.length > 0) {
          ...
          console.log(
            `[건강기능식품 검색] ✅ 의약품 탭에서 ${realMedicineResults.length}건 발견 - 탭 이동 안내`
          );
          return {
            results: [],
            suggestion: {
  ...
  async analyzeMedicineImage(
    ...
    try {
      ...
      const analysisResult = await geminiClient.analyzeMedicineImage(
        imageBase64
      );
      ...
      console.log(
        ...
        analysisResult.success
          ? `${analysisResult.totalCount}개 약품 감지`
          : "분석 실패"
      );
      ...
      if (!analysisResult.success || analysisResult.medicines.length === 0) {
        return {
          ...
          message:
            ...
            "이미지에서 약품을 인식할 수 없습니다. 약품명이 잘 보이도록 다시 촬영해주세요.",
          detectedMedicines: [],
          verifiedMedicines: [],
        };
      }
      ...
      for (const medicine of detectedMedicines) {
        ...
        const apiResults = await this.externalApiClient.getMedicineInfo(
          medicine.name,
          3
        );
        ...
        if (apiResults && apiResults.length > 0) {
          ...
          const pillResults =
            await this.externalApiClient.getPillIdentificationInfo({
      ...
      console.log(
        `[약품 이미지 분석] 완료 - 검증됨: ${
          verifiedMedicines.filter((m) => m.verified).length
        }건`
      );
   ...
   * 검색 결과에서 약 직접 등록
   */
  async addMedicineFromSearch(userId: string, medicineData: any) {
    ...
    const normalizeDetailFields = () => {
      ...
      detailedData.depositMethodQesitm = normalize(
        detailedData.depositMethodQesitm
      );
    };
    ...
    if (needsDetailFetch && itemSeq) {
      ...
      try {
        ...
        if (detailedData._isHealthFunctionalFood) {
          const healthFoodDetail =
            await this.externalApiClient.getHealthFoodDetail(itemSeq);
          if (healthFoodDetail) {
            detailedData.efcyQesitm =
              healthFoodDetail.efcyQesitm || detailedData.efcyQesitm;
            detailedData.useMethodQesitm =
              healthFoodDetail.useMethodQesitm || detailedData.useMethodQesitm;
            detailedData.atpnWarnQesitm =
              healthFoodDetail.atpnWarnQesitm || detailedData.atpnWarnQesitm;
            detailedData.atpnQesitm =
              healthFoodDetail.atpnQesitm || detailedData.atpnQesitm;
            detailedData.intrcQesitm =
              healthFoodDetail.intrcQesitm || detailedData.intrcQesitm;
            detailedData.seQesitm =
              healthFoodDetail.seQesitm || detailedData.seQesitm;
            detailedData.depositMethodQesitm =
              healthFoodDetail.depositMethodQesitm ||
              detailedData.depositMethodQesitm;
          ...
          const eDrugDetail = await this.externalApiClient.getMedicineInfo(
            itemName,
            1
          );
          if (
            eDrugDetail &&
            eDrugDetail.length > 0 &&
            eDrugDetail[0].itemSeq === itemSeq
          ) {
            const detail = eDrugDetail[0];
            if (detail.efcyQesitm) detailedData.efcyQesitm = detail.efcyQesitm;
            if (detail.useMethodQesitm)
              detailedData.useMethodQesitm = detail.useMethodQesitm;
            if (detail.atpnWarnQesitm)
              detailedData.atpnWarnQesitm = detail.atpnWarnQesitm;
            if (detail.atpnQesitm) detailedData.atpnQesitm = detail.atpnQesitm;
            if (detail.intrcQesitm)
              detailedData.intrcQesitm = detail.intrcQesitm;
            if (detail.seQesitm) detailedData.seQesitm = detail.seQesitm;
            if (detail.depositMethodQesitm)
              detailedData.depositMethodQesitm = detail.depositMethodQesitm;

            console.log(`✅ [약 등록] e약은요 상세정보 조회 완료`);
          } else {
            // 2️⃣ 허가정보 API 상세정보 조회
            const detailApiData =
              await this.externalApiClient.getDrugApprovalDetail(itemSeq);
            if (detailApiData) {
              detailedData.efcyQesitm =
                detailApiData.EE_DOC_DATA || detailedData.efcyQesitm;
              detailedData.useMethodQesitm =
                detailApiData.UD_DOC_DATA || detailedData.useMethodQesitm;
              detailedData.atpnWarnQesitm =
                detailApiData.NB_DOC_DATA || detailedData.atpnWarnQesitm;
              detailedData.seQesitm =
                detailApiData.SE_DOC_DATA || detailedData.seQesitm;
              detailedData.depositMethodQesitm =
                detailApiData.DEPOSIT_METHOD_QESITM ||
                detailedData.depositMethodQesitm;
        ...
        console.log(`✅ [약 등록] 최종 상세정보:`, {
          efcyQesitm: detailedData.efcyQesitm
            ? `있음(${detailedData.efcyQesitm.length}자)`
            : "null",
          useMethodQesitm: detailedData.useMethodQesitm
            ? `있음(${detailedData.useMethodQesitm.length}자)`
            : "null",
        });
      } catch (detailError) {
    ...
    try {
      ...
      if (geminiApiKey) {
        ...
        // 약품 정보 분석
        aiAnalyzedInfo = await geminiClient.analyzeMedicineInfo(
          itemName,
          detailedData
        );
        ...
        if (
          !detailedData.useMethodQesitm ||
          detailedData.useMethodQesitm.length < 10
        ) {
          aiScheduleInfo = await geminiClient.analyzeMedicineSchedule(
            itemName,
            detailedData
          );
        ...
        console.log(
          `✅ [약 등록] 토큰 절약 강화 정보 생성 완료 - 카테고리: ${enhancedInfo.category}`
        );
      }
    } catch (aiErr) {
    ...
    if (aiScheduleInfo) {
      ...
      console.log(
        `✅ [약 등록] AI 기반 복용 정보 설정: ${dosage}, ${frequency}`
      );
    }
  ...
  async getMyMedicines(userId: string, activeOnly: boolean = true) {
    ...
    return data.map((record, index) => {
      ...
      try {
        ...
        console.warn(
          `[getMyMedicines] qr_code_data 파싱 실패 (ID: ${record.id}):`,
          err.message
        );
      }
      ...
      const efcyQesitm =
        sanitize(record.efcy_qesitm) ||
        sanitize(qrData.efcyQesitm) ||
        sanitize(aiInfo.efficacy);
      ...
      const atpnWarnQesitm =
        sanitize(record.atpn_warn_qesitm) ||
        sanitize(qrData.atpnWarnQesitm) ||
        sanitize(aiInfo.precautions);
      const atpnQesitm =
        sanitize(qrData.atpnQesitm) || sanitize(aiInfo.precautions);
      const intrcQesitm =
        sanitize(record.intrc_qesitm) ||
        sanitize(qrData.intrcQesitm) ||
        sanitize(aiInfo.interactions);
      const seQesitm =
        sanitize(record.se_qesitm) ||
        sanitize(qrData.seQesitm) ||
        sanitize(aiInfo.sideEffects);
      const depositMethodQesitm =
        sanitize(record.deposit_method_qesitm) ||
        sanitize(qrData.depositMethodQesitm) ||
        sanitize(aiInfo.storageMethod);

      // 복용량/횟수 보완 (AI 분석 결과 우선)
      const dosage =
        sanitize(record.dosage) ||
        sanitize(aiScheduleInfo.dosagePerTime) ||
        sanitize(qrData.dosage);
      const frequency =
        sanitize(record.frequency) ||
        (aiScheduleInfo.timesPerDay
          ? `1일 ${aiScheduleInfo.timesPerDay}회`
          : null) ||
      ...
      const result = {
        ...
        // 차트용 추가 메타데이터
        _hasDetailedInfo: !!(efcyQesitm || seQesitm || intrcQesitm),
      ...
      if (index === 0) {
        console.log(`[getMyMedicines] 첫 번째 약품 (${result.itemName}):`, {
          "DB efcy_qesitm": record.efcy_qesitm
            ? `있음(${record.efcy_qesitm.length}자)`
            : "null",
          "DB use_method_qesitm": record.use_method_qesitm
            ? `있음(${record.use_method_qesitm.length}자)`
            : "null",
          "DB atpn_warn_qesitm": record.atpn_warn_qesitm
            ? `있음(${record.atpn_warn_qesitm.length}자)`
            : "null",
          "최종 efcyQesitm": result.efcyQesitm
            ? `있음(${result.efcyQesitm.length}자)`
            : "null",
          "최종 useMethodQesitm": result.useMethodQesitm
            ? `있음(${result.useMethodQesitm.length}자)`
            : "null",
          "최종 atpnWarnQesitm": result.atpnWarnQesitm
            ? `있음(${result.atpnWarnQesitm.length}자)`
            : "null",
  ...
  async analyzeInteraction(medicineIds: string[], foodName: string) {
    ...
    for (const medicine of medicines) {
      ...
      const hasInteraction = foodInteractions.some(
        (food) => foodName.includes(food) || food.includes(foodName)
      );
  ...
  async updateMedicineRecord(userId: string, recordId: string, updates: any) {
    ...
    if (updates.timeSlots) {
      ...
      let qrData = {};
      try {
        qrData = existingRecord.qr_code_data
          ? JSON.parse(existingRecord.qr_code_data)
          : {};
      } catch (err) {
        console.warn(
          `[updateMedicineRecord] qr_code_data 파싱 실패:`,
          err.message
        );
      }
      ...
      qrData["aiScheduleInfo"] = {
        ...
        dosagePerTime:
          updates.dosage || qrData["aiScheduleInfo"]?.dosagePerTime || "1정",
        recommendation: `사용자가 설정한 복용 시간대: ${updates.timeSlots
          .map((s) => {
    ...
    console.log(
      `✅ [약 복용 시간 업데이트] ID: ${recordId}, 시간대: ${updates.timeSlots?.join(
        ", "
      )}`
    );
    return data;
   ...
   */
  async analyzeAllMedicineInteractions(
    userId: string,
    ...
    if (userProfile && userProfile.age && userProfile.gender) {
      console.log(
        `[약물 상관관계 분석] 환자 정보: ${userProfile.age}세, ${
    ...
    const drugDetailsPromises = medicines.map(async (medicine: any) => {
      ...
      try {
        const qrData = medicine.qr_code_data
      ...
      let cachedData = null;
      if (itemSeq && entpName) {
        cachedData = await this.supabaseService.getMedicineDetailCache(
          itemSeq,
          entpName
        );
        if (cachedData) {
      ...
      const [info, pillInfo, approvalInfo] = await Promise.all([
        ...
        this.externalApiClient.getPillIdentificationInfo({
          itemName: medicine.name,
          numOfRows: 3,
        }),
        this.externalApiClient.getDrugApprovalInfo({
          itemName: medicine.name,
          numOfRows: 3,
        }),
      ]);
      ...
      const pillData =
        Array.isArray(pillInfo) && pillInfo.length > 0 ? pillInfo[0] : null;
      const approvalData =
        Array.isArray(approvalInfo) && approvalInfo.length > 0
          ? approvalInfo[0]
          : null;
    ...
    const analyzedMedicineInfo = await geminiClient.analyzeMedicineInfoBatch(
      medicineInfoBatch
    );
    console.log(
      `[약물 상관관계 분석] ✅ ${analyzedMedicineInfo.length}개 약품 분석 완료`
    );

    // Step 2: 약물 상호작용 분석
    console.log(`[약물 상관관계 분석] Step 2: 약물 상호작용 분석...`);
    const analysisResult = await geminiClient.analyzeAllDrugInteractions(
      drugDetails,
      userProfile
    );
    ...
    console.log(
      `  - 위험한 조합: ${analysisResult.dangerousCombinations?.length || 0}개`
    );
    console.log(
      `  - 주의 필요: ${analysisResult.cautionCombinations?.length || 0}개`
    );
    console.log(
      `  - 긍정적 효과: ${analysisResult.synergisticEffects?.length || 0}개`
    );
    ...
    (analysisResult.synergisticEffects || []).forEach((effect: any) => {
      ...
      const medicineIds = effect.drugs
        .map(
          (drugName: string) => medicines.find((m) => m.name === drugName)?.id
        )
        .filter(Boolean);
    ...
    console.log(
      `[약물 상관관계 분석] 네트워크 도표용 interactions 생성: ${interactions.length}개`
    );
    ...
    if (allFromCache) {
      console.log(
        `[약물 상관관계 분석] 캐시에서 모든 정보 조회 (API 호출 0회)`
      );
    } else {
      console.log(
        `[약물 상관관계 분석] 캐시: ${cacheInfo.fromCache}개, API: ${cacheInfo.fromAPI}개`
      );
    }
  ...
  async analyzeAllMedicineInteractionsStream(
    ...
    try {
      ...
      console.log(
        `[약물 분석 캐시] 캐시 키: ${cacheKey} (연령대: ${ageGroup})`
      );
      ...
      const drugDetailsPromises = medicines.map(async (medicine) => {
        const qrCodeData = medicine.qr_code_data
          ? JSON.parse(medicine.qr_code_data)
          : {};
        ...
        if (itemSeq && entpName) {
          const cached = await this.supabaseService.getMedicineDetailCache(
            itemSeq,
            entpName
          );
          if (cached) {
